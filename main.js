...
$("#cpass_btn").click(function(event){
		event.preventDefault();
	...
	$('body').delegate('.cpassword','click',function(event){
		event.preventDefault();
	...
	$("#login").click(function(event){
		...
		$.ajax({
			url: "login.php",
			method: "POST",
			data: {userLogin:1,email:email, pwd:pwd,cap:cap},
			success: function(data){
				if(data=="true"){
					window.location.href="profile.php";
				}
...
$('body').delegate('#area_sort','click',function(e){
		e.preventDefault();
		$(this).css('color','red');
		$('#pop_sort').css('color','black');
		$.ajax({
			url: "action.php",
			method: "POST",
			data: {getProduct:1,area_sorted:1},
			success: function(data){
				$('#get_product').html(data);
			}
		})
